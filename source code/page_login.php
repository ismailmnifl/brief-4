...
<?php
//include file connect for do conction with server
include 'connect.php';
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $comnd = $con->prepare("SELECT personne_formamteur.ID_FORMATEUR, CONCAT(personne.PRENOM ,' ', personne.NOM)  as Full_Name  from personne INNER JOIN personne_formamteur on personne.ID_PERSON = personne_formamteur.ID_PERSON  WHERE personne.EMAIL=? and personne.PASSWORD = ? ");
	...
	if ($Count >0){
		...
		if ($Count >0){
			...
			//echo "salam hada id ETD ".$_SESSION['ID_ETD']."hada ETD ".$row['ID_PERSON'] ." Email : ".$_SESSION['Email_ETD'];
			header('Location: Etudiant_pg.php?dash=0'); // Redirect To Dashboard Page
			...
			if ($Count >0){
				...
				$err="Your Email or Your Password Wrong";
			}
			
		}

	}
    // echo("<script>alert('Message et est Envoyer');</script>");
    // header("url=Page_contactUS.php");
}
?>
	...
	<div class="contant">
		<div class="svg_Part">
			<svg id="b368e7d6-7c4a-408c-9a35-0514749a7e1a" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"
				width="869.99994" height="520.13854" viewBox="0 0 869.99994 520.13854">
				<path
				...
				<rect x="427.49994" y="107.5" width="2" height="304" fill="#ccc" /></svg>
		...
		<div class="sing">
			...
			<br><br><br>
			<input type="email" name="email" class="em" placeholder="Your email" ID="email">
			<input type="password" name="pass" class="em" placeholder="Your password" ID="password">


			<Label ID="Err" runat="server"></Label>
			<input type="submit" value="Login" class="si">
			<br>
