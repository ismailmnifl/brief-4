<?php 
	...
	if(!empty($_SESSION['ID_ETD'])){
    	...
    	$results2 = mysqli_query($db,"SELECT classe_etd.ID_CLASSE_ETD,classe_etd.NOM,personne_formamteur.ID_FORMATEUR,CONCAT(personne.PRENOM ,' ', personne.NOM) AS Full_Name FROM classe_etd  INNER JOIN personne_formamteur on personne_formamteur.ID_FORMATEUR=classe_etd.ID_FORMATEUR INNER JOIN personne_etud on personne_etud.ID_ClasseETD=classe_etd.ID_CLASSE_ETD RIGHT JOIN personne on personne.ID_PERSON = personne_formamteur.ID_PERSON WHERE personne_etud.ID_ETUDIANT = $ID_ETD");
	...
	<div class="contant">
		<div class="dashb">
			<a href="Etudiant_pg.php?dash=1"><div>Profile</div></a>
